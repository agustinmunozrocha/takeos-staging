-- ============================================================================
-- Migración: guardar_proyecto — persistir he_config en el presupuesto por fila
-- Autor: BD Expert (respuesta al handoff de Code). Reconstruida con UTF-8 limpio
-- por Code, desde la función de la Pasada 1 + las dos líneas de he_config (idéntica
-- en lógica a la enviada por el BD Expert; el pegado original llegó con mojibake).
--
-- QUÉ: el bloque PRESUPUESTO POR FILA de guardar_proyecto omitía la columna
-- he_config (desglose de horas extra). hora_extra (costo cacheado) sí se guardaba
-- -> totales correctos, pero el desglose no se persistía ni se podía re-editar.
-- NO es regresión: ninguna versión anterior la escribía (0/230 filas en prod).
--
-- CONTRATO: el cliente ya manda 'heConfig' en cada upsert; este cambio solo hace
-- que el RPC la lea y la guarde. NO requiere cambios de cliente.
--
-- CÓMO: CREATE OR REPLACE (mismo retorno jsonb -> sin DROP, preserva grants
-- {authenticated, postgres}; por eso no lleva REVOKE/GRANT). Aditivo, bajo radio:
-- solo el cuerpo de la función (he_config en INSERT y UPDATE). No toca tablas,
-- RLS, policies ni grants.
-- ============================================================================
